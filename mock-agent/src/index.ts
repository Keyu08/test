...
async function parseBody(req: any): Promise<RequestBody> {
...
const server = createServer(async (req, res) => {
  ...
  try {
    if (pathname === '/health' && method === 'POST') {
      ...
      const mockSchema = {
        columns: [
          { name: 'group', type: 'categorical' as const, non_null_count: 20, null_count: 0 },
          { name: 'value', type: 'numeric' as const, non_null_count: 20, null_count: 0 },
        ],
        n_rows: 20,
        candidate_x: [],
        candidate_y: ['value'],
      ...
      const mockResult = {
        ...
        methods_text:
          'Two-sample t-test was performed to compare means between groups. Data were first assessed for normality using Shapiro-Wilk test and homogeneity of variance using Levene test.',
        explain_bundle: {
      ...
      if (format === 'png') {
        ...
        res.writeHead(200);
        res.end(
          '<svg width="400" height="300" xmlns="http://www.w3.org/2000/svg"><text x="50" y="150">SVG Export Stub</text></svg>'
        );
      } else if (format === 'pdf') {
